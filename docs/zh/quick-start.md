...
会创建 `WB_DATA_PATH/config.json` 和 `WB_DATA_PATH/workspace/`（默认 `WB_DATA_PATH=~/.weavbot`），并启动**交互式配置向导**，依次完成：

1. **服务商选择** — 从 [models.dev](https://models.dev) 拉取已知服务商与模型，选择服务商、模型并填写 API 密钥
2. **渠道配置** — 配置聊天渠道（Telegram、Discord、飞书、钉钉、Slack、QQ、企业微信（Wecom）、微信（Wechat）、Mochat）的凭证
