...
- **Built-in tools** — `read_file`, `write_file`, `edit_file`, `list_dir`, `glob_file`, `grep_file`, `shell`, `load_media`, `fetch`, `message`, `spawn`, `cron` (plus MCP tools)
...
- **Subagent tools** — file tools (read/write/edit/list_dir/glob/grep), `shell`, `load_media`, `fetch`; **excludes** `message`, `spawn`, `cron`
