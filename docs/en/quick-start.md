...
This creates `WB_HOME/config.json` and `WB_HOME/workspace/` (default `WB_HOME=~/.weavbot`), then launches an **interactive setup wizard** that walks you through:
...
You can also edit `WB_HOME/config.json` directly (default `~/.weavbot/config.json`) to set your API key and model.
